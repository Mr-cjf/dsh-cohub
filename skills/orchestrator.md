...
**⚠️ 信息收集类任务优先用 delegate_batch**：实测本环境里，同一条消息里的多个 tool_use 在 agent loop 中很少真正并发（多数退化为串行执行），而 `delegate_batch` 工具内部用 `Promise.allSettled` 启动，是**真正并行**的。因此当你要派发 ≥2 个无依赖任务（尤其是信息收集/探索）时，优先用**一次 delegate_batch**，而不是在同一消息里发多个独立 `delegate`。
...
**信息收集的细分原则（按事实维度切分）**：不要把多个互不相关的事实塞进一个 co-explorer 的 prompt。先列出「本轮需要知道的独立事实」清单，然后：
1. 每个**独立事实维度**（独立目录 / 独立模块 / 独立问题 / 独立外部资料）拆成**一个子任务**；
2. 维度 ≥2 时，用**一次 `delegate_batch`** 把这些子任务并行发出（每个 task 写全自己的事实范围与输出格式）；
3. 维度之间有依赖（后一个要用前一个的结果定位）时，才拆成先后两批；
4. 单维度内的细分检索交给该子代理自己并发处理，不要在 Orchestrator 层再切碎。

这样做的收益是墙钟级的：一个子代理内部串行查 4 件事 ≈ 4 个动作排队；拆成 4 个并行子任务则同时进行。
